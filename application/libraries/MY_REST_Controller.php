...
class MY_REST_Controller extends REST_Controller
{
    ...
    **/
    ...
    /** Overwrite the set_response_paginate
    * As it's default converting all string to number if string contains only numbers
    * @param NULL $http_code Status code of api
    * @param NULL $message Message to be sent for api
    * @param NULL $data Data to be sent in api response
    * @param NULL $total_rows count total data of api
    * @param NULL $per_page total data per page of api
    * @param NULL $current_page identity for current page number of api
    * @param NULL $from identity start page of api
    * @param NULL $to identity next page of api
    * @param FALSE $continueExe If want to continuwe execution 
    **/
    public function set_response_paginate($http_code = NULL, $message=NULL, $data = NULL, $total_rows = null, $per_page = null, $current_page = null, $from = null, $to = null, $continueExe = FALSE)
    {
        $response = array("status_code"=>$http_code,"message"=>$message,"data"=>$data, "total_rows" => $total_rows, "per_page" => $per_page, "current_page" => $current_page, "from" => $from, "to" => $to);
        $this->response_without_numeric_check($response, $http_code, $continueExe);
    }
